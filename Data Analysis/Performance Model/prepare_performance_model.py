...
for cluster in  performance_models_per_cluster:
    ...
    tree_reg = DecisionTreeRegressor(random_state=42, min_samples_leaf=6)
    ...
    # NN Model
    NN = MLPRegressor( activation='logistic', max_iter=5000, solver='lbfgs', hidden_layer_sizes=(6, 10), random_state=42)
    ...
    ####################################################
    ####################################################
    #The Best Regression Model is Decision Tree Regressor
    # Decision Tree Regressor
    ...
    # Training Accuracy
    TR_prediction = tree_reg.predict(strat_train_set)
    TR_mse_train = mean_squared_error(train_labels, TR_prediction)
    TR_rmse_train = np.sqrt(TR_mse_train)
    print "\t\tTraining Data: TR Root Mean Square Error = {0:0.2f}".format(TR_rmse_train)

    # Testing Accuracy
    TR_prediction = tree_reg.predict(strat_test_set)
    TR_mse_test = mean_squared_error(test_labels, TR_prediction)
    TR_rmse_test = np.sqrt(TR_mse_test)
    print "\t\tTesting Data: TR Root Mean Square Error = {0:0.2f}".format(TR_rmse_test)
    ...
    ax1.plot(TR_prediction, c='b', label='Prediction (RMSE={0:0.2f})'.format(TR_rmse_test))
    ...
    tree_reg = DecisionTreeRegressor(random_state=42, min_samples_leaf=6)
    tree_reg.fit(norm_features, labels.values.ravel())
    # You have to create the folder 'Output Models' manually
    with open('./Output Models/Cluster {} - Decision Tree Regressor Model'.format(cluster_no),'w') as file:
    ...
    prediction = tree_reg.predict(norm_simulation)

    #Figures - Part 2
    fig1 = plt.figure(cluster_no+3)
    ax1 = fig1.add_subplot(111)

    ax1.plot(range(0, len(prediction)), prediction, c='b', label= 'Prediction (RMSE={0:0.2f})'.format(TR_rmse_test))
